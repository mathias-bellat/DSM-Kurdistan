...
for (depth in new_increments) {
  ...
  data.OC <- bi_class(compared_map, x = OC, y = SoilGrid.OC , style = "quantile", dim = 3)
  ...
  data <- cbind(compared_map,data.pH[15],data.Nt[15],data.OC[15],data.Sand[15],data.Silt[15],data.Clay[15])
  names(data)[15:ncol(data)] <- c("bi_class_pH", "bi_class_Nt", "bi_class_OC", "bi_class_Sand", "bi_class_Silt", "bi_class_Clay")
  ...
  for (i in 3:8) {
    ...
    map <- ggplot() +
      ...
      labs(title = paste0("Prediction maps vs. SoilGrid model for ", depth),
  ...
  List_map[[depth]] <- compared_map
}

Table_summary


transform_prof <- function(df, prof_id){
  df %>%
    as.data.frame() %>%
    mutate(stat = rownames(df)) %>%
    ...
    mutate(
      model = ifelse(grepl("^SoilGrid", var), "SoilGrid", "Prediction"),
      variable = gsub("SoilGrid\\.|^", "", var),
      profil = paste0(prof_id)
    ) %>%
    
    dplyr::select(profil, model, variable, stat, value)
}

df_long <- imap_dfr(Table_summary, transform_prof)


df_final <- df_long %>%
  pivot_wider(
    id_cols = c(variable, stat),
    names_from = c(profil, model),
    values_from = value
  ) %>%
  arrange(variable, stat) 

df_placement <- round(df_final[3:8], digit = 2)
df_final <- cbind(df_final[1:2], df_placement[,c(2,1,4,3,6,5)])

# Change the "html" to "latex"
kable(df_final, "latex", escape = FALSE) %>%
  kable_styling(full_width = FALSE) %>%
  scroll_box(width = "100%")

save(List_map, Table_summary, df_final, file = "./export/save/SoilGrid.RData")
rm(list=setdiff(ls(), c("make_subdir", "increments", "List_map", "new_increments")))
